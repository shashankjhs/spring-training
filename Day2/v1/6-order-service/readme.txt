...
- Steps to use Openfeign
 ...
 4. Declare methods in openfeign -
      - Signature of method must match with remote method we are interested in calling(Methid name can be change)
      - Annotate these methods with exact same 
 
 
 Enable feign clients by annotating application class with @EnableingFeignClients
